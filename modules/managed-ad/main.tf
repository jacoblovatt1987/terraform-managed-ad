## Examples taken from:
# https://registry.terraform.io/modules/aws-samples/windows-workloads-on-aws/aws/1.1.1/submodules/managed-ad
# 

locals {
  ds_managed_ad_type               = "MicrosoftAD" # Static input to force only MAD deployments
  ds_managed_ad_admin_secret_suffix = "admin"
}
...
resource "aws_secretsmanager_secret" "mad_admin_secret" {
  name       = "${var.ds_managed_ad_directory_name}_${local.ds_managed_ad_admin_secret_suffix}"
